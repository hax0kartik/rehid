...
void Titles::FilterOutTWLAndUpdate()
{
    for(int i = 0; i < m_count; i++)
    {
        if((((m_array[i] >> 32) & 0x8000) == 0) && (((m_array[i] >> 32) & 0xE) == 0))
            m_filteredvector.push_back(m_array[i]);
    }
}
...
void Titles::PopulateSMDHArray()
{
    ...
    std::string descstr;
    for(uint32_t i = 0; i < m_filteredvector.size(); i++)
    {
        data.clear();
        data.shrink_to_fit();
        descstr.clear();
        descstr.shrink_to_fit();
        u32 mediatype = m_filteredvector[i] == m_gamecardid ? MEDIATYPE_GAME_CARD : MEDIATYPE_SD;
        u32 archivepath[4] = {(u32) (m_filteredvector[i] & 0xFFFFFFFF), (u32) ((m_filteredvector[i] >> 32) & 0xFFFFFFFF), mediatype, 0x00000000};
...
void Titles::ConvertDescsToC2DText()
{
    static C2D_TextBuf staticbuf;
    m_c2ddescvector.resize(m_descvector.size());
    staticbuf = C2D_TextBufNew(4096);
    for(int i = 0; i < m_descvector.size(); i++)
    {
